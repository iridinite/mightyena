    ...
    public class Gen3Pokemon {
        ...
        public void Save() {
            ...
            if (Exists) {
                ...
                if (!boxed) {
                    Nature nature = Nature.ByIndex((int)(Personality % 25));
                    ...
                    Buffer.BlockCopy(BitConverter.GetBytes(CalculateStat(level, Species.BaseAttack, IVAttack, EVAttack, nature.GetModifier(Statistic.Attack))), 0, frame, offset + 90, 2);
                    Buffer.BlockCopy(BitConverter.GetBytes(CalculateStat(level, Species.BaseDefense, IVDefense, EVDefense, nature.GetModifier(Statistic.Defense))), 0, frame, offset + 92, 2);
                    Buffer.BlockCopy(BitConverter.GetBytes(CalculateStat(level, Species.BaseSpeed, IVSpeed, EVSpeed, nature.GetModifier(Statistic.Speed))), 0, frame, offset + 94, 2);
                    Buffer.BlockCopy(BitConverter.GetBytes(CalculateStat(level, Species.BaseSpAttack, IVSpAttack, EVSpAttack, nature.GetModifier(Statistic.SpAttack))), 0, frame, offset + 96, 2);
                    Buffer.BlockCopy(BitConverter.GetBytes(CalculateStat(level, Species.BaseSpDefense, IVSpDefense, EVSpDefense, nature.GetModifier(Statistic.SpDefense))), 0, frame, offset + 98, 2);
                }
